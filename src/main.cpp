...
int main() {
  ...
  //Following variables must be here because they need to be remembered in between calls
  int lane = 1; // My lane 0: left, 1: middle, 2: right Lane=4m
  ...
  h.onMessage([&ref_vel, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&lane](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		// previous_path_x is whatever was left over after the car had consumed its  share of points
		...
		for(int i=1;i<= 50-previous_path_x.size();i++){
		    double N = (target_dist/(0.02*ref_vel/2.24)); //2.24 is mph to meter per sec
		    double x_point = x_add_on+(target_x)/N;
		    double y_point = s(x_point);
		    x_add_on = x_point;
		    double x_ref = x_point;
		    double y_ref = y_point;
		    //Rotate back to normal and shift
